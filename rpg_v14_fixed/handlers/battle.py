...
async def battle_action_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not admin and action not in ("flee",):
        try:
    ...
    if action == "potion":
        ...
        log.append(f"🧪 Minum potion +{heal} HP! (Monster tidak menyerang)")
        # BUG FIX: potion tidak memicu serangan balik monster (konsisten dengan dungeon.py)
        state["turn"] += 1
        save_player(user.id, player)
        _sbs(context, user.id, state)
        await _show_battle(query, player, state)
