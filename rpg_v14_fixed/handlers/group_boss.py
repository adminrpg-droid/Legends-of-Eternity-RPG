...
async def _finish_raid(context, chat_id: int, msg_id: int, sess: dict):
    ...
    for uid, p in sess["players"].items():
        ...
        if boss_dead:
            ...
            # BUG FIX: update quest weekly_coin_earned setelah boss raid selesai
            # bonus_gold hanya ada untuk killer — gunakan base_gold saja yang pasti ada
            extra_gold = base_gold if uid == killer_id else 0  # killer dapat x2, sudah di-track di weekly_coin_earned atas
            player_db = update_quest_progress(player_db, "weekly_coin_earned", base_gold + extra_gold)
