...
async def _ask_sell_price(query, source: str, slot: str, item_id: str, context=None):
    ...
    if len(cb_gold) > 64 or len(cb_diamond) > 64:
        if context is not None:
            context.bot_data[f"{user.id}_mkt_currency_source"]  = source
            context.bot_data[f"{user.id}_mkt_currency_slot"]    = slot
            context.bot_data[f"{user.id}_mkt_currency_item_id"] = item_id
