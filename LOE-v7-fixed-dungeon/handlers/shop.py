...
async def _show_pet_shop(query, player: dict):
    ...
    if current_pet and current_pet in PET_SHOP:
        current_pet_name = PET_SHOP[current_pet]["name"]
    elif current_pet:
        current_pet_name = current_pet.replace('_', ' ').title()

    lines = [
        "🐾 *PET SHOP ETERNITY*\n"
        f"💎 Diamond: *{diamond}* | 🪙 Gold: *{coin:,}*\n"
        f"🐾 Pet sekarang: *{current_pet_name}*\n"
    ]
    buttons = []
    available_count = 0
    for pet_id, pet in PET_SHOP.items():
        # Skip pet yang sudah dimiliki — hapus dari tampilan shop
        if current_pet == pet_id:
            continue
        rarity = pet["rarity"]
        icon = RARITY_COLOR.get(rarity, "⚪")
    ...
    if available_count == 0:
        lines.append("✅ *Semua pet telah kamu miliki!*\n")
