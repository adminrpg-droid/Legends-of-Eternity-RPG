...
def _calc_scaled_boss_stats(base_hp, base_atk, base_def, player_count: int) -> dict:
    ...
    hp_scale  = 10.0 + (pc - 1) * 3.0
    atk_scale = 4.0  + (pc - 1) * 0.5
    def_scale = 3.0
    return {
...
async def _run_raid(context, chat_id: int, msg_id: int):
    ...
    # Scale boss berdasarkan jumlah AKTUAL pemain yang join
    scaled = _calc_scaled_boss_stats(boss["base_hp"], boss["base_atk"], boss["base_def"], player_count)
...
async def _finish_raid(context, chat_id: int, msg_id: int, sess: dict):
    ...
    for uid, p in sess["players"].items():
        ...
        if boss_dead:
            ...
            # ── Drop untuk SEMUA peserta (10% item biasa, 0.1% evo+SSSR) ──
            char_class = player_db.get("class", "warrior")
            all_drop_txt = ""
            # 10% chance item biasa untuk semua
