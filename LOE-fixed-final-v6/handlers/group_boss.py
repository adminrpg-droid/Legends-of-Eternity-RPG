...
def _calc_scaled_boss_stats(base_hp, base_atk, base_def, player_count: int) -> dict:
    ...
    [FIX v8] Scaling diperkuat agar stat boss benar-benar naik signifikan
    saat lebih banyak pemain bergabung.
    """
    pc = max(1, player_count)
    # HP naik 15x untuk 1 pemain, +5x per pemain tambahan
    hp_scale  = 15.0 + (pc - 1) * 5.0
    # ATK naik 5x untuk 1 pemain, +0.8x per pemain tambahan
    atk_scale = 5.0  + (pc - 1) * 0.8
    # DEF naik proporsional dengan pemain
    def_scale = 3.0  + (pc - 1) * 0.4
    return {
...
async def _join_raid(query, user, chat_id: int, context):
    ...
    try:
        ...
        effective_atk = _get_enhance_atk(player)
        effective_def = _get_enhance_def(player)
    except Exception:
    ...
    sess["players"][user.id] = {
        ...
        "atk":      effective_atk,
        "def":      effective_def,  # BUG FIX: include armor enhance bonus
        "spd":      player["spd"],
