...
def _give_item_to_buyer(player: dict, item_id: str, item: dict, source: str):
    ...
    if item_type in ("weapon", "armor"):
        ...
        equip[slot] = item_id
        # Tambahkan ke inventory agar konsisten dengan sistem shop
...
async def market_action_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action.startswith("market_currency_"):
        ...
        if meta == "ctx":
            source  = context.user_data.pop("mkt_currency_source", "equip")
            slot    = context.user_data.pop("mkt_currency_slot", "weapon")
            item_id = context.user_data.pop("mkt_currency_item_id", "")
        else:
            parts   = meta.split("|")
...
async def _ask_sell_price(query, source: str, slot: str, item_id: str, context=None):
    ...
    if len(cb_gold) > 64 or len(cb_diamond) > 64:
        if context is not None:
            context.user_data["mkt_currency_source"]  = source
            context.user_data["mkt_currency_slot"]    = slot
            context.user_data["mkt_currency_item_id"] = item_id
