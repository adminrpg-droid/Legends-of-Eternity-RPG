...
async def _buy_equipment(query, player: dict, user_id: int, item_id: str, eq_type: str):
    ...
    if old_id:
        ...
        # BUG FIX: tambahkan ke inventory HANYA jika belum ada
        # Saat beli item pertama kali item sudah masuk inventory,
        # sehingga tidak boleh di-increment lagi saat dilepas karena ganti item baru
        inv = player.setdefault("inventory", {})
        if inv.get(old_id, 0) == 0:
            inv[old_id] = 1
