...
async def _do_reset_player(query, uid: int):
    ...
    create_player(uid, name, player.get("class", "warrior"), player.get("gender", "male"), player.get("username", ""))
    await query.answer(f"✅ {name} berhasil di-reset!", show_alert=True)
...
async def _reset_all_menu(query):
    ...
    await query.edit_message_text(
        "⚠️ *PERINGATAN!*\n\nIni akan mereset data SEMUA pemain!\n_(Data admin tidak terpengaruh)_\n\nTekan tombol konfirmasi untuk melanjutkan.",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup(buttons)
    )
...
async def _do_reset_all(query):
    ...
    for uid_str, p in players.items():
        ...
        if is_admin(uid):
            continue
        create_player(uid, p.get("name", f"ID:{uid}"), p.get("class", "warrior"), p.get("gender", "male"), p.get("username", ""))
        count += 1
    await query.edit_message_text(
        f"✅ *Reset Selesai!*\n🗑️ {count} pemain telah di-reset.",
        parse_mode="Markdown",
