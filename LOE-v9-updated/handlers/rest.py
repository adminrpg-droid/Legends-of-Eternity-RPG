...
REST_TICK_SECONDS = 3        # regen tiap 3 detik (dipercepat)
REST_HP_REGEN     = 40       # HP per tick (dipercepat)
REST_MP_REGEN     = 35       # MP per tick (dipercepat)
REST_MAX_DURATION = 300      # maks 5 menit istirahat
REST_COOLDOWN     = 30       # cooldown setelah berhenti istirahat (detik)
...
async def rest_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Cek apakah sedang istirahat (dengan anti-stuck: max 10 menit)
    if player.get("is_resting"):
        rest_start = player.get("rest_start", 0)
        if now - rest_start > REST_MAX_DURATION + 300:
            # Sesi istirahat sudah kadaluarsa (bot mungkin restart) - reset
