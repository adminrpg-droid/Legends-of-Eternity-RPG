...
async def _process_dg_action(query, player: dict, dstate: dict, action: str, context, user_id: int):
    ...
    if action == "potion":
        ...
        log.append(f"🧪 Potion +{heal} HP! (Monster tidak menyerang)")
        # BUG FIX: potion tidak memicu serangan balik monster (konsisten dengan battle.py)
        dstate["turn"] += 1
        save_player(user_id, player)
        _sds(context, user_id, dstate)
        ...
        e_atk = player.get("atk", 0)
        wpn_id = player.get("equipment", {}).get("weapon")
        if wpn_id:
            from items import get_item
            item_d = get_item(wpn_id)
            if item_d:
                wpn_atk = item_d.get("stats", {}).get("atk", 0)
                e_lv = get_enhance_level(player, "weapon")
                # BUG FIX: tambahkan weapon base ATK + enhance bonus
                e_atk += wpn_atk + enhance_stat_bonus(wpn_atk, e_lv)
    ...
    # Kurangi cooldown skill setiap turn kecuali saat pakai skill atau potion
    if action not in ("skill", "potion") and player.get("skill_cooldown", 0) > 0:
        player["skill_cooldown"] -= 1

    # ── Monster dies — cek SEBELUM boss regen agar boss tidak regen saat hp <= 0 ─
    if monster["current_hp"] <= 0:
        gold  = int(random.randint(*monster.get("gold", (20, 50))) * 1.3)
        exp   = int(monster.get("exp", 50) * 1.3)

        player["coin"]  = player.get("coin", 0) + (gold * 2 if is_boss else gold)
        player["exp"]   += exp
        player["kills"] += 1
        earned = gold * 2 if is_boss else gold
        ...
        await query.edit_message_text(
            ...
            f"💰 +{gold * (2 if is_boss else 1)} Gold\n"
    ...
    if is_boss and monster["current_hp"] > 0 and action not in ("potion",):
    ...
    # ── Boss counter attack ──────────────────────────────────────
    if is_boss and action not in ("potion",):
