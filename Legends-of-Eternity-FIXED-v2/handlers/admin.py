...
async def _admin_help_inline(query):
    text = (
        ...
        "`/setlevel <id> <level>` — Set level pemain\n\n"
        "🗑️ *RESET*\n"
...
async def setlevel_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/setlevel <user_id> <level> — Set level pemain."""
    if not is_admin(update.effective_user.id):
        await update.message.reply_text("❌ Akses ditolak!")
        return
    args = context.args
    ...
    if level < 1 or level > 999:
        await update.message.reply_text("❌ Level harus antara 1–999!")
        return
    player = get_player(uid)
    if not player:
        await update.message.reply_text(f"❌ Pemain ID `{uid}` tidak ditemukan!", parse_mode="Markdown")
    ...
    player["level"] = level
    # Recalculate exp_needed based on target level
    base_exp_needed = 120
    ...
    # Recalculate stats from scratch based on class base stats + level gains
    from database import CLASS_STATS
    char_class = player.get("class", "warrior")
    base = CLASS_STATS.get(char_class, CLASS_STATS["warrior"])
    level_gains = level - 1
    player["max_hp"] = base["max_hp"] + level_gains * 8
    player["max_mp"] = base["max_mp"] + level_gains * 5
    player["atk"]    = base["atk"]    + level_gains * 2
    player["def"]    = base["def"]    + level_gains * 1
    player["spd"]    = base["spd"]    + level_gains * 1
    ...
    await update.message.reply_text(
        ...
        f"❤️ HP: {player['max_hp']} | ⚔️ ATK: {player['atk']} | 🛡️ DEF: {player['def']}",
        parse_mode="Markdown")
