...
async def _run_raid(context, chat_id: int, msg_id: int):
    ...
    # Scale boss HP berdasarkan jumlah pemain (lebih ringan)
    scale  = 1.0 + (player_count - 1) * 0.2
    boss["hp"]         = int(boss["hp"] * scale)
    boss["max_hp"]     = boss["hp"]
    boss["current_hp"] = boss["hp"]
    boss["atk"]        = int(boss["atk"] * (1.0 + (player_count - 1) * 0.08))
...
async def _finish_raid(context, chat_id: int, msg_id: int, sess: dict):
    ...
    for uid, p in sess["players"].items():
        ...
        if boss_dead:
            # Semua yang ikut dapat XP dan gold — reward ditingkatkan
            base_exp  = int(boss.get("exp", 500) * 2.0 / max(1, len(sess["players"])))
            base_gold = int(random.randint(*boss.get("gold", (100, 300))) * 2.5 / max(1, len(sess["players"])))
